...
def iter_messages(snapshot: Path, conversation_id: str | None, start: int | None, end: int | None, keyword: str | None, limit: int | None) -> list[dict]:
    ...
    with connect(path) as connection:
        for table in MESSAGE_TABLES:
            ...
            scan_limit = min(max(limit * 50, 1000), 50000) if keyword and limit is not None else limit
            limit_clause = f" LIMIT {scan_limit}" if scan_limit is not None else ""
            sql = f'SELECT {",".join(fields)} FROM "{table}"{where} ORDER BY send_time DESC{limit_clause}'
