...
hook('MD5_Update', {
  onEnter(args) {
    ...
    if (length === 16 || length === 24 || length === 32) {
      report(args[1], 16, 'OpenSSL-MD5_Update-' + length);
    }
  }
});

hook('EVP_DigestUpdate', {
  onEnter(args) {
    const length = args[2].toUInt32();
    if (length === 16 || length === 24 || length === 32) {
      report(args[1], 16, 'OpenSSL-EVP_DigestUpdate-' + length);
    }
  }
});
...
hook('md5_process', {
  onEnter(args) {
    report(args[1], 16, 'bundled-md5_process-block');
  }
});
...
def capture(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        agent_source = FRIDA_AGENT
        if args.generic_hooks_only:
...
def main() -> int:
    ...
    capture_parser.add_argument(
        "--generic-hooks-only",
        action="store_true",
        help="跳过版本相关的固定偏移 hook，仅监听 CommonCrypto/sqlite 导出函数",
    )
